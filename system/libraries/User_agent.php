...
class CI_User_agent {
	...
	private function _compile_data()
	{
		...
		foreach (array('_set_robot', '_set_browser', '_set_mobile', '_set_ipad') as $function)
		{
	...
	/**
	 * Set the Table Device
	 *
	 * @access	private
	 * @return	bool
	 */
	private function _set_ipad()
	{
		if (is_array($this->ipads) AND count($this->ipads) > 0)
		{
			foreach ($this->ipads as $key => $val)
			{
	 ...
	 * @return	bool
	 */
	public function is_ipad($key = NULL)
	{
		if ( ! $this->is_ipad)
		{ return FALSE; }
		...
		{ return TRUE; }
		// Check for a specific robot
		return array_key_exists($key, $this->ipads) AND $this->ipad === $this->ipads[$key];
	}

	// --------------------------------------------------------------------
	/**  * Is Mobile *  * @access	public * @return	bool  */
