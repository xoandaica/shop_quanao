...
//            'cate'=>$data['cate']->alias,
        ...
        $this->LoadHeader($seo);
        // load profuct lien quan
        $this->load->model('f_productmodel');
        $dataProductLienquan['product_lienquan'] = $this->f_productmodel->getNewProduct();
        $data['product_lienquan'] = $this->load->view('widgets/product_newest', $dataProductLienquan, true);
...
//            $data['like_view']=$this->session->set_userdata('liked');
    ...
    public function like2() {
        if (isset($_POST['id'])) {
            ...
            $this->f_productmodel->Update_Where('users', array('id' => $this->session->userdata('userid')), array('liked' => json_encode($liked)));
            echo json_encode($rs);
        }
    }
    ...
    public function un_like2() {
        if (isset($_POST['id'])) {
            ...
            $this->f_productmodel->Update_Where('users', array('id' => $this->session->userdata('userid')), array('liked' => json_encode($new)));
            echo json_encode($rs);
        }
    }
