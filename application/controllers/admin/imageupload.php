...
//        'ads_right'=>'Quảng cáo phải',
    ...
    public function banners()
    {
        ...
        if($this->input->get()){
            ...
            $config['total_rows']           = $this->imagemodel->countBanner($where,$this->language);
            $config['per_page']             = 50;
            $config['uri_segment'] = 4;

            $this->pagination->initialize($config);
            $data['list'] = $this->imagemodel->getBannerList($where, $config['per_page'], $this->input->get('per_page'),$this->language);
            $data['total_rows']=$config['total_rows'];

        }else{

            $config['base_url'] = base_url('vnadmin/imageupload/banners');
            $config['total_rows'] = $this->imagemodel->countBanner(null,$this->language); // xác định tổng số record
            $config['per_page'] = 50; // xác định số record ở mỗi trang
            $config['uri_segment'] = 4; // xác định segment chứa page number
            $this->pagination->initialize($config);

            $data['list'] = $this->imagemodel->getBannerList(null,$config['per_page'], $this->uri->segment(4),$this->language);
        }
